...
function <%= appName %>_setup() {
	 ...
	 * to change '<%= appName %>' to the name of your theme in all the template files.
	 */
	load_theme_textdomain( '<%= appName %>', get_template_directory() . '/languages' );
	...
	register_nav_menus( array(
		'primary' => esc_html__( 'Primary', '<%= appName %>' ),
	) );
...
function <%= appName %>_widgets_init() {
	register_sidebar( array(
		...
		'id'            => 'sidebar-1',
		'description'   => esc_html__( 'Add widgets here.', '<%= appName %>' ),
